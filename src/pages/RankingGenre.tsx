...
export default function RankingGenre() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const rows = await fetchPerformancesByGenre(activeGenre);
        if (!cancelled) {
          setItems(rows);
  ...
  const onPickGenre = (g: Genre) => {
    setActiveGenre(g);

    const qs = new URLSearchParams(searchParams);
    qs.set("genre", g);

    // ✅ 지역은 페이지 전환 시 유지(표시하지는 않음)
    if (!qs.get("region")) qs.set("region", activeRegion);

    setSearchParams(qs);
  };

  const goRegionRanking = () => {
    // ✅ 현재 선택 장르/지역을 쿼리로 넘겨서 유지
    const qs = new URLSearchParams();
    qs.set("region", searchParams.get("region") ?? activeRegion);
    qs.set("genre", searchParams.get("genre") ?? activeGenre);
    navigate(`/ranking/region?${qs.toString()}`);
  };
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {GENRES.map((g) => (
          <Chip key={g} active={activeGenre === g} onClick={() => onPickGenre(g)}>
            {g}
          </Chip>
        ))}
      </div>
